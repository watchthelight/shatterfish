...
A sixth was found by continuous integration, after three reviews and every local run had passed: on the Linux runner, where Gradle's output capture is slower, the actor thread blocked on the console's lock while echoing a log line at the end of its turn, HotSpot counted that park as a wait, `awaitPark` returned before the thread reached its monitor, and the next step's wait-site check named the foreign lock, `ReentrantLock$NonfairSync`, which is exactly what it was for. A park now counts only when the thread is parked at a known site, the accounting uses the count the stepper confirmed rather than an assumed one, and the boot silences libGDX's console echo of the game log to errors only, since the harness reads the log through `GLog`'s own listener.
...
The second review passed the fence, with its own probe of 340 fenced frames finding no frame where the thread was woken and not waited out or waited out without being woken, and asked for six things, all taken (the first two as refined by the third review):
...
- The wait-site check by identity as well as class, which is exact outside the test JVM's identity-hash pin, where every identity hash is the same value and the class remains the only discriminator. The review also showed a prediction of *which* sprite from `Actor.current` can be wrong in a Rig process, because `Pushing`'s render-thread callback under this very curse can spawn a mob and so change `Actor.chars()` between the buff's copy of it and the stepper's; the third review then showed the prediction bought nothing and could fail a legitimate Run, so it is gone, and with it the stepper's read of `Actor.current`. The check now asks only that the waited-on object is one the stepper is about to hold, and names the site from the frame that called `wait`.
...
The third review passed the fence on HotSpot at the tag and found seven refinements, all taken:

- The between-frames self-check read the wait count after the monitors were released, so on a JVM that reports a notified thread as still waiting a short turn could park again before the read and be absorbed. The count is now read inside the fence and checked in three places: unchanged during a frame, exactly one more across a frame that woke the thread, unchanged between frames.
- The which-sprite prediction from `Actor.current` was a false-positive source that bought no soundness: the fence needs only "the reported lock is the thread or a held sprite", and a spawn from a render-thread callback under the curse could make a legitimate wait fail the Run. The prediction is gone, the site tripwire is the frame that called `wait`, and the stepper reaches one upstream field instead of two.
- The reflection rule matched calls only; a method reference or a reflective call to `getDeclaredField` itself passed it, both demonstrated. The rule is now by dependency on `java.lang.reflect` and `java.lang.invoke`, with a narrow allowance for the dynamic proxy, and the bite test carries both evasions.
- The package anchor walked the classes directory only; a class file placed under `src/main/resources` shipped in an upstream package and loaded, demonstrated. Both anchors, harness and brain, now walk every output directory and refuse a class file anywhere in the source tree.
- The scene-switch refusal keyed on the stepper's thread rather than the scene's; a real `GameScene` driven by an external stepper was not covered. It now asks the scene, through the stepper's one reflective read, and the stepper refuses to start a thread while the scene has a live one.
- Story prose and the battery row for the wait-site check corrected to say what the identity-hash pin makes undetectable in principle.
...
| M14 the stepper reaches a second field the ledger does not name | `HarnessReflectionTest` fails: the reach and the declared set differ |
...
| M17 a harness class reaches a private field by method reference | `HarnessReflectionTest`'s rule fails on the dependency |
| M18 a class file placed under `src/main/resources` in an upstream package | `HarnessPackageAnchorTest` fails, naming the file |
| M19 a class file placed in the source tree | `HarnessPackageAnchorTest` fails, naming the file |
...
- **Reflection into upstream.** One private static is reached, from one class, and a rule holds it there; a second rule keeps every harness class file out of upstream's packages. The ledger still cannot see either route; `docs/UPSTREAM.md` says so and names the field, and the tests check the name and the packages.
...
- **The wait sites are knowledge of the tag.** Three are known and checked by the frame that called `wait`. An upgrade that adds one fails a step by name; the fix is to teach the stepper the new site, which is a reading of upstream, not a hook.
- **The fence reads HotSpot's reporting.** That a notified thread reads `BLOCKED` on the notifier's side is HotSpot behaviour, not a language guarantee. The three wait-count checks in the stepper turn a JVM that reports differently into a failure at the next frame rather than a silent race.
