...
| Criterion | Outcome |
|---|---|
| The binding is installed before any texture class initializes, and atlases load through the image path that needs no graphics context | **Met.** `HeadlessBoot` installs the no-op binding as its second step, before any game class that builds a texture is touched, and `HeadlessScene`'s constructor refuses to exist unless `NoOpGL.isNoOp(Gdx.gl)` holds, so the order is a property of the code rather than of which test ran first. `HeadlessSceneTest` asserts both, and asserts the level's tile atlas was read as a `Pixmap` with a size. The `Texture` class's own statics turned out to be compile-time constants (`Texture.java:34-39`), so class initialization never touches GL; every constructor does (`:47`) |
...
- `shatterfish/harness/src/test/java/org/shatterfish/harness/scene/`: `SceneDrawParityTest`, `HeadlessSceneTest`, and two fixtures, `FreshRun` (the game's own start and continue paths) and `DrawCounter` (a counting generator on the game's stack).
...
- `HeadlessDriver` boots through `HeadlessBoot` and no longer exits; story 1.4 puts the loop in it.
...
- Docs: ADR-0015 amended; `docs/UPSTREAM.md` row 5 and a paragraph on the harness's two reflective reads; three rows in `docs/rules/game-loop.md` and two in `docs/rules/rng.md`; `docs/architecture.md`.
...
ADR-0015 chose "a `Scene` subclass that creates the Groups `GameScene` creates", on the reasoning that sprite and emote constructors draw random numbers. Reading `GameScene`'s statics before writing a line showed that the groups are the smaller half of the problem. `GameScene.scene` is package-private and assigned only inside `GameScene.create()` (`GameScene.java:159`, `:240`); every static game code calls is gated on it; and these are not all drawing:

- `add(Mob, float)` registers the mob as an actor and links its sprite only when a scene exists (`:1153-1162`). Under any other scene a spawned mob is added to `level.mobs` and never acts.
- `add(Heap)` marks a heap as not counting for the exploration bonus only then (`:1132-1138`).
- `show(Window)` shows a window only then (`:1358`), which is the Prompt mechanism story 1.4 needs.
- `effectOverFog` and `addSprite` dereference the scene with no guard at all (`:1149`, `:1192`).
...
`SceneStepper` fences every frame. It holds the actor thread's monitor across the frame, so the scene's own notify cannot let the thread run before the frame ends; it holds the monitor of the sprite the thread is waiting on, so a movement ending inside the frame cannot either; it decides whether the thread was woken by repeating the scene's own rule while the thread still cannot run; and after the frame it polls the JVM's count of the thread's waits (`ThreadInfo.getWaitedCount`) until the thread has entered its next wait. That count is the one signal that distinguishes "still parked" from "notified and not yet running". The thread is also started by the stepper before the first frame, where the scene would otherwise start it in the middle of the first `update()`.

Three of those four pieces were found by failure:

1. Without the sprite fence, movement releases let the thread run mid-frame. Every parity run diverged (mutation M4 below).
...
3. With the thread created by the scene mid-frame, one run in about a hundred and eighty reached its second Input wait two frames late. The stepper now starts the thread itself, mirroring `GameScene.java:866-880`.

The remaining race (3) was never caught by the mutation battery afterwards (M5, M6 pass); it is reported as found by reasoning and by a scratch probe that ran the script many times per JVM, not as a property the committed tests demonstrate.
...
- **Identity hash order.** The game orders actors, mobs and weighted choices by `HashSet` iteration; HotSpot assigns identity hashes from a per-thread random state; two runs of one seed in two JVMs are two different games. This is ledger row 6, story 1.16's. Until it lands, the harness test JVM runs with `-XX:hashCode=2`, which pins every identity hash and turns hash order into insertion order. `SceneDrawParityTest` asserts the pin is in effect, so a dropped flag fails rather than flakes. Story 1.16 deletes both.
- **A static initializer draws.** `WindParticle`'s wind angle is drawn when the class is first used, once per process (`WindParticle.java:41`). The first Run in a process draws one more than every later one. A Rig Run is its own process and pays it every time; in one process it shifts every comparison by one, so the parity fixture throws its first Run away. It is the only static initializer under `core` that draws.
- **The guidebook is placed from an unseeded generator.** `EntranceRoom.placeEarlyGuidePages` pushes a generator with no seed, on purpose, so that meta-progression does not perturb the rest of the layout (`EntranceRoom.java:103-118`). Generating a floor twice from one seed does not give the same floor. `docs/rules/rng.md` already had the row; this story met it. The parity fixture generates once and every compared Run resumes the saved game through the game's own continue path (`InterlevelScene.java:733-747`). The determinism story owns the real answer.
- **The journal is loaded once per process, and level generation reads it.** Which guide pages are found decides what the first floors drop (`RegularLevel.java:561-575`); `Journal.loadGlobal()` returns early after its first call, and so does `Badges.loadGlobal()`. The fixture resets both and reloads them from a fresh profile, which is what the hero-select screen does before every game (`HeroSelectScene.java:106-107`). Story 1.15 owns the Profile.
...
- **A click is accepted on the frame the hero becomes ready** because `Hero.ready()` reaches `selectCell`, which sets `cellSelector.enabled` on the actor thread (`GameScene.java:1570`). The script relies on it.
...
`./gradlew build -Pshatterfish.mobile=off`: green, 43 tests across 9 suites. `mkdocs build --strict`: clean.

The reference fingerprint for the committed seed and script, from the real `GameScene` and matched exactly by `HeadlessScene` three times over: 72 draws to create, 4065 draws during 60 Input waits over 352 frames, and every listed field equal. Eighty further runs of the same script across four JVMs, in a scratch probe that compared per-wait state, all matched their reference.
...
| M4 no sprite gate | both parity tests fail, three of three, with frame counts from 976 to 1572 against 352 |
| M5 wake decided after the fence is released | passes three of three. **Not demonstrated.** Found by a scratch probe at about one run in twenty on an earlier floor |
| M6 actor thread started by the scene mid-frame | passes three of three. **Not demonstrated.** Found by the same probe at about one in a hundred and eighty |
| M7 fixture keeps the last Run's journal | passes. **Moot under the snapshot fixture**: compared Runs resume one save, so a stale journal is stale for all of them equally |
...
| M11 `HeadlessScene` is not `GameScene.scene` | `HeadlessSceneTest` fails three times (no actor registered, no click accepted, no turn) and the parity fixture cannot end its actor thread |
| M12 no wait for the park after a wake | both parity tests fail, three of three |
...
- The story was told to inherit story 1.1's boot recipe and not its readiness polling. `SceneDrawParityTest`'s script reads `hero.ready` between frames to decide when to click. It is a test script standing in for story 1.5's Input-wait detection, and it reads the flag only while the actor thread is parked, which is not the race ADR-0015 rejected; but it is polling, and story 1.5 replaces it.
- The manual `:desktop:debug` check was not run. This story edits no upstream file.

## Known limitations, handed forward

- **The pin.** `-XX:hashCode=2` makes the harness's tests exact and the game's own iteration order insertion order. It is not a property of a Rig Run, which will need row 6.
- **Reflection into upstream.** Two private statics are reached (`SceneStepper`). The ledger cannot see reflection from `shatterfish/`; `docs/UPSTREAM.md` now says so and names both. There is no rule yet that forbids a third.
...
- **`HooksVanillaTest.the_guarded_sites_are_inert_with_no_scene`** now runs, in the same JVM, after a real `CellSelector` has been assigned; its claim is checked against a non-null selector when scene tests ran first. The test still passes and still checks the guard, but not with the null it names.
...
- The determinism story meets `EntranceRoom.placeEarlyGuidePages` and `WindParticle`.
