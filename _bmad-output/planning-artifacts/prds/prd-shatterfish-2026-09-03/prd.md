...
- **Hook budget.** At most ten Hooks in v1, each listed in the registry and counted by a test against `docs/UPSTREAM.md`. The ledger is in ADR-0008: the mobile-module guard in `settings.gradle` (exists); the Hooks registry class; the game-scene seam (creation and destruction, which is also where the Observer re-registers its log listener); read-only accessors for private state the screen shows; the hero Input-wait notification and the headless-scene guards; identity-order removal for determinism; the input gate that lets PAUSED ignore hero input; the sprite-wait bypass for the fastest speed mode; render-thread draw routing so an Overlay Run replays; and the key-binding and settings-section registration. Exceeding ten requires an ADR.

`[NOTE FOR PM]` This budget was eight until the architecture's reviewer gate (session 12) showed that two required capabilities cannot ship inside eight: PAUSED ignoring hero input (FR-40) needs a gate the game consults, because the game reinstalls its own cell listener on every wake-up and its key-hold path bypasses listeners entirely; and hotkeys (FR-42) with the v2 Pause-on settings section (FR-45) need a registration point, because the game's action bindings live in a private static map. Each added row is one line at its site.
