...
| Rig statistics: GSPRT port over the Per-pair statistic (trinomial: better, equal, worse on the Composite outcome), bounds, burn-in, calibration, recalibration per tag, e-process alternative; Seed-set sizes; where `standard` runs | E3 | Research recommendation 3; brief addendum, Statistics; PRD FR-20, FR-21, open question 11 |
...
**Run log record (one JSONL line per Input wait):**
...
```
./gradlew :rig:run --args="--brain <name> [--baseline <name>] --seeds <set|N> [--seed-start K] [--parallel P] --out <dir>"
```
...
- Canonical follow-on: the highest verified Score of a winning Run within one registered pass, per challenge-flag set (SM-2).
