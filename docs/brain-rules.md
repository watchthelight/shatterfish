...
| 14 | A click on a transition cell with no enemy in view travels, and on the hero's own transition cell it is the Descend input; a sealed floor refuses every transition, so the descend Policy walks onto the exit when it leaves a floor and never from a sealed floor. | `descend` | [game-loop: "`Hero.handle(cell)` sets `curAction` by cell content"](rules/game-loop.md) |
...
| 43 | Every regular floor of the main branch places one food when it is built, and the guaranteed drops of a set of floors -- two potions of strength, three scrolls of upgrade, an arcane stylus -- on its floors before the boss; so the descend Policy leaves a floor hungry with no food held, and expects the drops still owed spread over the floors left in the set. | `descend` (when to leave) | [levels: "On non-boss branch-0 floors, `Level.create` pre-adds"](rules/levels.md) |
| 44 | On a regular floor a wandering enemy spawns every 50 turns while the floor holds fewer than its limit; so staying on a floor has a cost, and the descend Policy leaves one it has overstayed. | `descend` (when to leave) | [combat: "`Level.mobCount()` sums"](rules/combat.md) |
| 45 | Regeneration heals 1 HP every 10 turns, and none while starving; so the descend Policy rests on the exit to full health before going down, and never while hungry or starving. | `descend` (rest) | [buffs: "Regeneration heals 1 HP"](rules/buffs.md) |
| 46 | A boss fight seals the floor, and while it is sealed no transition travels; so the descend Policy does not enter on a sealed floor. | `descend` | [levels: "`Level.locked` is set by"](rules/levels.md) |
