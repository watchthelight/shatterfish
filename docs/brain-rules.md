...
| 39 | A potion of healing heals `(int)(0.8 * HT + 14)`, a quarter of what is left each turn (at least 1) after a one-turn drink; its buff draws no icon, and a second heal replaces what is left of the first rather than adding. So the heal Policy drinks only when the hero is missing at least the first turn's heal, and not again until the first heal would be spent. | `heal` | [buffs: "A potion of healing, drunk"](rules/buffs.md) |
...
| 41 | A sleeping mob shows the sleep icon, and a sleeping mob does not attack; so the heal Policy counts only enemies without it as threatening. | `heal` (threshold) | [combat: "A sleeping mob never attacks"](rules/combat.md) |
