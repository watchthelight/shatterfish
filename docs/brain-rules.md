...
| 17 | A hunting mob attacks when it can attack its enemy, which is when it is adjacent, so an enemy beside the hero is one that is fighting it. | `fight` | [combat: "A hunting mob attacks when"](rules/combat.md) |
