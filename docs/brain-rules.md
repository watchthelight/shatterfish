...
| 35 | The upgrade window's "Upgrade" button upgrades the item the reading Action chose, and its "Back" reopens the selector, so answer-prompt presses "Upgrade". | `answer-prompt` (upgrade) | [ui: "The upgrade window, which the scroll of upgrade and magical infusion open"](rules/ui.md) |
| 36 | The guess window names the item and draws its category's unidentified types as icons, a tap shows the guess button with that type's name, and closing spends nothing, so answer-prompt taps the likeliest type by the Beliefs, then the guess button, or leaves. | `answer-prompt` (guess) | [ui: "The stone of intuition's guess window is titled with the chosen item's name"](rules/ui.md) |
