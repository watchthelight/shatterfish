...
the NetHack symbolic champions used no search, that Long et al.'s properties (leaf correlation,
bias, disambiguation factor) predict whether sampled search helps a game, and that none of those
numbers exist for SPD (research §4, recommendation 6). The bootstrap allows this decision to be
...
| Long et al. properties | Leaf correlation, bias and disambiguation factor computed with random playouts from the same snapshots, following the recipe in research §4 | whether sampled search is expected to beat the one-ply model; low disambiguation means determinizations mislead |
...
Fairness rule for any winner: the search consumes `Observation`, `Belief` and `Snapshot` values
through `api`; its rollouts run in `harness` behind `Redeterminer`; the Brain never holds a game
object (AD-7, AD-9).
...
- Long et al.'s recipe assumes a perfect-information game and the properties are ill-defined
  here. Mitigation: they are explanatory only; the choice rule never gates on them.
