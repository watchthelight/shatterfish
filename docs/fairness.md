...
| The bestiary | `Codex.Tactics` (`api`) carries one enemy class's tags from `tactics/bestiary.json` -- speed, attack, reach, flight, immunities, what it inflicts, the state it is created in, the derived approach, whether it can be outrun, what breaks its pursuit -- plus two Codex facts the rig joins in: the alignment the mob table creates the class with and the depths the spawn rotation places it on. All of it is general game knowledge, derived from the pinned code and cited on the bestiary's cards, the same for every Run; the rig reads the file beside the Codex (`CodexKnowledge`), and the Brain looks an entry up only by the name the actors section shows and the depth the header shows (`Codex.Knowledge.tactics`), never by an enemy's class, which the screen does not draw. It replaces the Brain's hand lists of passive enemies, enemies that shoot or fly, and immovable enemies | done: `TacticsTest`, `BestiaryKnowledgeTest`, `BestiaryTagsTest` (the bestiary's lever 0) |
