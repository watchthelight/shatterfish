...
## 3. Facts about the upstream game (verified against `v3.3.8` in session 10; corrections are recorded in `docs/codebase-map.md`, rules in `docs/rules/`)

- Java + libGDX with a scene-graph layer called Noosa. Gradle multi-module: `SPD-classes`, `core`, `desktop`, `android`, `ios`, `services` (plus four service sub-modules); upstream compiles for Java 11 on libGDX 1.14.0. The Android/iOS modules must not be required for desktop or headless builds; the Android SDK is not installed and must not be needed.
- **There are no data files.** Every enemy, item, drop table, and spawn weight is Java code (a mob's constructor sets HP, defense, EXP; its `damageRoll()` is a method). Player-facing text lives in `core/src/main/assets/messages/**/*.properties`, keyed by class name. `Assets.java` lists every asset path; sprite frame sizes live in each `*Sprite` class. The Adventurer's Guide and lore pages come from `Document.java` plus `journal.properties`. The in-game changelog lives in the `ui.changelist` package (`ChangesScene` drives it); the v3.3.8 entry carries no date, and no 4.0 entry exists at the tag. Six asset paths are literal strings outside `Assets.java`.
- **Seeding is partial.** Level generation, item appearances, room orders and the `Generator` deck seeds are fixed by the run seed (under `seed+1` in `Dungeon.init`, then per floor by `seedForDepth`); the general-purpose generator that combat rolls use is the unseeded base of `com.watabou.utils.Random`, and `Dungeon.init` ends with `Random.resetGenerators()`, so the harness must seed the base generator *after* `Dungeon.init` or runs are not reproducible. Further sources to control: actor tie-breaks by `HashSet` identity-hash order, `Random.chances(HashMap)` inside seeded level generation, `bones.dat`, `journal.dat` and `settings.xml` state read during generation, an unseeded `pushGenerator()` for guide pages on floors 1 and 2. `Random.NormalIntRange` is triangular (sum of two uniforms) and is what damage and armor rolls use; the hit roll itself is uniform.
- **Game loop.** `Actor.process()` advances actors on a dedicated "SHPD Actor Thread" started from `GameScene.update()`; the hero waits for input by returning `false` from `act()`; input arrives as `Hero.handle(cell)` / `Hero.curAction`; item use is `Item.execute(hero, action)`. Rendering is one libGDX thread and `Game.runOnRenderThread` is `Gdx.app.postRunnable`; level changes run on an `InterlevelScene` worker thread. Turns resolve through sprite callbacks: `Actor.process()` waits on a moving `CharSprite`, and the hero's attack, operate, zap and read actions end in animation callbacks fired on the render thread; `Hero.ready()` and `Hero.interrupt()` dereference `AttackIndicator.instance` and `GameScene.cellSelector`. A `Window` blocks the player, not the actor thread. Game state is static singletons (`Dungeon`, `Actor`) and is mostly unsynchronized. `Statistics.duration` advances only in `Actor.fixTime` (save, level change, death); elapsed turns are `duration + Actor.now()`.
- **Player visibility is already computed by the game**, because it has to draw it. `Dungeon.observe()` fills `Level.heroFOV` and `visited` (mind vision, magical sight, blindness as a 3x3 block, and the darkness-reduced view distance are handled inside `Level.updateFieldOfView`); `mapped` is written only by the Scroll of Magic Mapping, Foresight searches and bundle load, and affects the fog layer, never the FOV. Mob sprites are drawn iff `heroFOV[mob.pos]`; an invisible char is still drawn at alpha 0.4. Secret doors render as walls until found; traps have a `visible` flag; heaps have a sticky `seen` flag. Items expose `name()`, `isIdentified()`, `levelKnown`, `cursedKnown` (wands also `curChargeKnown`, rings `isKnown()`). Unseen mobs leak existence through the log and their sprite bounding boxes through `CellSelector`; the Observer must use none of that. Only buffs with an icon reach the HUD, and the exact hunger value is never shown. Build the Observation from these, never from raw model fields.
- **Guarantees are in code.** Strength potions: `Dungeon.posNeeded()` (two per five-floor set, at most one per floor). Upgrade scrolls: `Dungeon.souNeeded()` (three per set). Both are `Dungeon.LimitedDrops` counters applied in `Level.create`, not a `Generator` deck. Special rooms guarantee their solution item on the same floor via `Level.addItemToSpawn()` (e.g., `TrapsRoom` → levitation, `PoolRoom` → invisibility), though another room's `findPrizeItem` may pull it into a chest. Item spawn weights and the category and item decks live in `Generator`. Enemy spawn tables per depth: `MobSpawner.getMobRotation(depth)`. Alchemy recipes: `Recipe` subclasses. Hit chance: `Char.hit` (uniform accuracy roll against uniform evasion roll). Boss floors are 5, 10, 15, 20, 25; `Level.seal` locks every transition from the arena trigger to the boss's death; the Amulet is placed on depth 26, and the Win is ending the Run with it (ascension optional).
- **UI toolkit.** `Window`, `Chrome` (16 nine-patch types), `RedButton`, `IconButton`, `Icons`, `PixelScene.renderTextBlock` (a platform TTF at sizes 5 to 12 UI pixels; the 3x5 `pixelFont` bitmap is only for the version and depth numbers), `StatusPane`, `Toolbar`, `GameScene`, `KeyBindings`, `SPDAction` (defaults in a private static map; F6 to F12 unbound). The UI pixel size is `PixelScene.defaultZoom` (3 for a 1280x720 window on the full UI). `GameScene`'s HUD fields are private, so an overlay needs a hook to reach them; the camera's `centerOffset` is vertical-only and overwritten by `layoutTags`. Save/load is `Bundle`; a fresh Profile matters because `bones.dat`, `journal.dat` and `settings.xml` feed level generation.
