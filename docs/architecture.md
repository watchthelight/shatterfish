...
| `brain` | `api` only | Beliefs, scripted policies, tactical search, strategic playbooks, evaluation. Identical code runs headless and in the overlay. Since story 4.1: `Brain` (arbitration over a priority list of `Policy`s, re-planned from each Observation), `Memory` (the Belief's bytes: what was seen, never what was intended), `Stream` (the Brain's own SplitMix64 randomness), and `BrainDecider`, the `api` `Deliberator` the Run loop drives and logs the Decision and Belief hash of. The rig names it `shatterfish`. Since story 4.4: a `Policy` ranks what it would do, and its next ranks are the Decision's alternatives; `Safety` (the Decision's flags, read off the screen); `Highlights` (the cells the chosen Action points at, handed over through `Deliberator.lastHighlights` into the wait record) |
