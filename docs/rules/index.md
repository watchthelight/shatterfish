...
place, the row is flipped to needs-review and its link is left at `v3.3.8`, where it was true. Of
the rows with citations, **233 still resolve to the same code** (145 of them at new line numbers)
and **80 are waiting to be re-read**. A needs-review row is not known to be wrong: it is known to
be unchecked, and the story that next depends on it re-reads it (E2's Codex pipeline is the bulk
buyer).
